...
def imc_Controller_Interface():
    st.header('Controlador de Modelo Interno (IMC)')
    ...
    with imc_config_col:
        ...
        imc_siso_tab_form() # Chamamos o formulário SISO diretamente
...
def imc_siso_tab_form():
    transfer_function_type = st.radio('**Tipo de Função de Transferência**',['Continuo','Discreto'],horizontal=True,key='imc_siso_transfer_function_type')
    ...
    num_coeff = st.text_input('Coeficientes **Numerador**:',key='siso_imc_num_coeff',help=help_text,placeholder='7.737')
    coefficients_validations(num_coeff)
    den_coeff = st.text_input('Coeficientes **Denominador**:',key='siso_imc_den_coeff',help=help_text,placeholder='0.6 , 1')
    coefficients_validations(den_coeff)

    delay_checkbox_col, delay_input_col = st.columns(2)
    with delay_checkbox_col:
        delay_checkbox=st.checkbox('Atraso de Transporte?',key='siso_imc_delay_checkbox')
    
    with delay_input_col:
        if delay_checkbox:
            delay_input = st.number_input(label='delay',label_visibility='collapsed',key='siso_imc_delay_input')
    ...
    if reference_number == 'Única':
        imc_siso_single_reference = st.number_input(
        ...
        with col23:
            imc_siso_multiple_reference3 = st.number_input(
                'Referência 3:', value=30.0, step=1.0, min_value=0.0, max_value=90.0, key='imc_siso_multiple_reference3')

        with col22:
            imc_siso_multiple_reference2 = st.number_input(
                'Referência 2:', value=30.0, step=1.0, min_value=0.0, max_value=90.0, key='imc_siso_multiple_reference2')

        with col21:
            imc_siso_multiple_reference1 = st.number_input(
                'Referência 1:', value=30.0, step=1.0, min_value=0.0, max_value=90.0, key='imc_siso_multiple_reference1')

        changeReferenceCol1, changeReferenceCol2 = st.columns(2)

        with changeReferenceCol2:
            siso_change_ref_instant3 = st.number_input(
                'Instante da referência 3 (s):', value=calculate_time_limit()*3/4, step=0.1, min_value=0.0, max_value=calculate_time_limit(), key='siso_imc_change_ref_instant3')
        ...
        with changeReferenceCol1:
            ...
            siso_change_ref_instant2 = st.number_input(
                ...
                key='siso_imc_change_ref_instant2')

    st.write('**Constante de Tempo de Malha Fechada ($\\tau_{mf}$):**')
    imc_mr_tau_mf1 = float(st.text_input('$\\tau_{mf}$', value="0.9", key='siso_imc_mr_tau_mf1'))

    if st.button('Iniciar', type='primary', key='imc_siso_button'):
        
        if reference_number == 'Única':
            imcControlProcessSISO(transfer_function_type,num_coeff,den_coeff,
                                imc_mr_tau_mf1,
                                imc_siso_single_reference, imc_siso_single_reference, imc_siso_single_reference)
        
        elif reference_number == 'Múltiplas':
            imcControlProcessSISO(transfer_function_type,num_coeff,den_coeff,
                                imc_mr_tau_mf1,
                                imc_siso_multiple_reference1, imc_siso_multiple_reference2,imc_siso_multiple_reference3,
                                siso_change_ref_instant2,siso_change_ref_instant3)

# A função imc_mimo_tab_form() foi removida.